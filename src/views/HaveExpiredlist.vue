<template>
  ...
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item style="font-size: 20px;">已过期</el-breadcrumb-item>
    </el-breadcrumb>
    <br>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="siPrepare" label="筹款期限(天)" width="120">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="danger" @click="handle(scope.$index, scope.row,2)">延期</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<!--    编辑-->
    <el-dialog style="font-size: 20px;width:1400px" :title="edit.titles" :visible.sync="dialogMergeFormVisible">
      <el-form :label-position="labelPosition" label-width="80px"   status-icon :inline="true" status-icon :model="SIssue" class="demo-form-inline">
        ...
        <el-form-item label="筹款期限">
          <el-input placeholder="筹款期限" :disabled="assge" v-model.number="SIssue.siPrepare"></el-input>
        </el-form-item>
        ...
        <el-form-item label="状态">
          <el-input placeholder="状态" v-if="siState1"  :disabled="assge" v-model="SIssue.siState"></el-input>
          <el-radio  v-if="siState2"  v-model="SIssue.siState" value="1" label="1">审核通过</el-radio>
          <el-radio v-if="siState2" v-model="SIssue.siState" value="2" label="2">审核不通过</el-radio>
        </el-form-item>
<!--        <el-form-item label="状态">-->
<!--          <el-radio v-model="SIssue.siState" label="1">审核通过</el-radio>-->
<!--          <el-radio v-model="SIssue.siState" label="2">审核不通过</el-radio>-->
<!--        </el-form-item>-->
    ...
    <!--    延期-->
    <el-dialog style="font-size: 50px;width:1000px;" :title="edit.titles" :visible.sync="dialogMergeFormVisible1">
      <el-form :label-position="labelPosition" label-width="120px"   status-icon :model="SIssue" class="demo-form-inline">
        <el-form-item label="延期的天数" >
          <el-input style="width: 60%" type="text" placeholder="延期的天数" v-model="SIssue.siPrepare"></el-input>&nbsp;天
        </el-form-item>
        <el-form-item label="温馨提示：">
          <label>有效期=目前到期时间+天数</label>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="doupdate">确定</el-button>
        <el-button @click="dialogMergeFormVisible1 = false">返回</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      doupdate(){
        ...
        console.log(this.SIssue.siPrepare)
        console.log(this.SIssue.siState)
        this.axios.post(url, this.SIssue).then((response) => {
          this.$message({
            message: "延期成功",
            type: 'success',
          });
          this.dialogMergeFormVisible1=false;
      ...
      handle(index, row,a){
        ...
        this.SIssue.siPrepare=row.siPrepare;
      ...
      handleEdit(index, row) {
        ...
        this.edit.titles='查看';
        this.siState1=true;
        this.SIssue.siState='已过期';
      },
      // 编辑
      handleupdate(index, row) {
        this.edit.titles='延期';
        this.dialogMergeFormVisible1=true;
        this.SIssue.siState='0';
        this.SIssue.siSubmit=this.date1(row,index);
      },
