<template>
  ...
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item style="font-size: 20px;">已还完借款</el-breadcrumb-item>
    </el-breadcrumb>
    <br>
    ...
    <el-dialog style="font-size: 20px;width:1400px" :title="edit.titles"  :visible.sync="dialogMergeFormVisible">
      <el-form :label-position="labelPosition" label-width="80px" status-icon :inline="true" status-icon :model="SIssue"
...
<script>
  export default {
    ...
    methods: {
      ...
      // formatDate(getTime) {
      //   var date = getTime; //时间戳为10位需*1000，时间戳为13位的话不需乘1000
      //   console.log(date)
      ...
      //   return Y + M + D+H + MI  + S;
      // },
      // getNextMonth(getDate,d,m) {
      //   var a=getDate
      //   var date=this.formatDate(a)
      //   var year = Number(date.substr(0, 4));
      ...
      //     month2 = '0' + month2;
      //   }
      //   var t2 = year2 + '-' + month2 + '-' + day2+' '+hourNum+':'+minuteNum+':'+secondNum;
      //   var index=parseInt((new Date(t2).getTime()-a.getTime())/1000/60/60/24)+1;
      //   var t=0.0005;
      //   if(d==3){
      //     t=0.00055
      //   }else if(d==6){
    ...
    created: function () {
      // this.getNextMonth(new Date(),1,10000);
